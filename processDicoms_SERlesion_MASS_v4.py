...
def transformDICOM_coords(LesionCenterPixelX, LesionCenterPixelY, spacing, origin, image_ori_pat, image_pos_pat):
	global scaled_cords
	...
	print "Situation dicom-2-vtk:"	
	#testing
	dicom_coords = array([-56, 24.2, 108, 1])
	vtk_image_coords = dicom_coords*IO.I
	print vtk_image_coords	
	
	print "Situation B:"	
	#testing
	scaled_ijk_coords = array([97.5, 124.8, 28, 1]).reshape(4,1)
	origin_scaled_ijk_coords = IO*(scaled_ijk_coords)
	print origin_scaled_ijk_coords
		
	return vtk_image_coords[0], vtk_image_coords[0]
...
def create_SER(selectMethod, numberS, img_folder, LesionID, SeriesID, eID, StudyID, abspath_ExamID, row, dicomInfo, tablefile_name, tablewriter, irow_lesionSegment):
	...
	if(selectMethod == '0'):
		...
		if(list_files[0]!='DIRCONTENTS.txt'):
			...
			path_filenameID = path_filenameID[0:-8]+'1.MR.dcm'
		else:
			path_filenameID = abspath_PhaseID+os.sep+list_files[0]
			path_filenameID = path_filenameID[0:-8]+'1.MR.dcm'
	...
	for i in range(1,numberS):
		...
		################### Process DICOM coords
		print '''\n Process DICOM coords:''' 
		print str("LesionCenterPixelX: ")+str( row[11])+'\n'+str("LesionCenterPixelY: ")+str( row[12])
		LesionCenterPixelX = row[11]
		LesionCenterPixelY = row[12]
		
		# transform DICOM XY coords to simply I,J,K=0 coords
		[cordi, cordj] = transformDICOM_coords(LesionCenterPixelX, LesionCenterPixelY, spacing, origin, image_ori_pat, image_pos_pat)
		print cordi, cordj
