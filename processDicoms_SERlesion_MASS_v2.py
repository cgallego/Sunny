...
def display_pick(image, spatial_res, slice_thickn, image_pos_pat, image_ori_pat, dicomReader, zplane):
...
def create_SER(img_folder, LesionID, SeriesID, eID, StudyID, abspath_ExamID, row, dicomInfo, tablefile_name, tablewriter, irow_lesionSegment):
	global DynVolstack, chosen_lesions_id, dicomInfo_series, abspath_SeriesID, lThre, uThre
	
	# Makedir of current loc
	os.chdir(abspath_ExamID)
	DynVolstack =  vtk.vtkImageAppendComponents() 
	DynVolstack.SetNumberOfThreads(1)
	...
	if os.path.exists('DynPhases'):
		...
		for phaseS in phases_series:
			...
			if( phaseS == 'S600'):
				...
				dicomReader_pre =  vtk.vtkDICOMImageReader()
				dicomReader_pre.SetDirectoryName( abspath_PhaseID )
				dicomReader_pre.Update()
				
				DynVol_pre = vtk.vtkImageData()				
				# Obtain pre contrast volume
				DynVol_pre.DeepCopy(dicomReader_pre.GetOutput())
				#DynVol_pre.SetOutputScalarTypeToFloat()
				#DynVol_pre.Update()
								
				# Get image from reader
				im_scalars = dicomReader_pre.GetOutput().GetPointData().GetScalars()
				dims = dicomReader_pre.GetOutput().GetDimensions()
					
				# check wether DicomTag corresponds to Image loaded in vtk
				print "\n... Reading %s" % str(phaseS)
				print "VTK Dimensions im.GetDimensions(): %d %d %d" % dims
				spacing =  dicomReader_pre.GetOutput().GetSpacing()
				...
				if(i==1):
				##############################################################################
					print "\n---------------------------------------------------------\nProcesing Substraction image %d " % i
					DynVol_early_pos = vtk.vtkImageData()
					
					# Obtain early post-contrast volume
					DynVol_early_pos.DeepCopy(dicomReader.GetOutput())
					...
					sub_pre_early.SetInput1(DynVol_early_pos)
					sub_pre_early.SetInput2(DynVol_pre)
					...
					zplane = display(image, image_pos_pat, image_ori_pat)
											
					DynVol_time_pos = vtk.vtkImageData()
					DynVol_time_pos.DeepCopy(dicomReader.GetOutput())
					...
					sub_pre.SetInput1(DynVol_time_pos)
					sub_pre.SetInput2(DynVol_pre)
					sub_pre.Update()
					...
					seeds = vtk.vtkPoints()
					seeds = display_pick(sub_pre.GetOutput(), spatial_res, slice_thickn, image_pos_pat, image_ori_pat, dicomReader, zplane)
					...
					thresh_sub.SetInputConnection(sub_pre.GetOutputPort())
					thresh_sub.Update()
					...
					print "\n---------------------------------------------------------\nProcesing Substraction image %d " % i
					DynVol_late_pos = vtk.vtkImageData()
														
					# Obtain early late-contrast volume
					DynVol_late_pos.DeepCopy(dicomReader.GetOutput())
					...
					sub_pre_late.SetInput1(DynVol_late_pos)
					sub_pre_late.SetInput2(DynVol_pre)
					sub_pre_late.Update()
		...
		############ Get number of lesions and their IDs
		# instead of asking get it from list
		...
		DynVol_late_pos.SetOutputScalarTypeToChar()
		DynVol_late_pos.Update()
...
title_range = tablewriter.getRangeByCells((1, 1), (1, 29)) #where j = number of columns 
tablewriter.formatRange(title_range, excel_writer.STYLE_GREY_CELL)

# start by writing headings 
#headings_DynSeries = ['StudyNumber', 'DicomExamNumber', 'MRN', 'ClinicalTrialID', 'StudyDate', 'SeriesDescription', 'SeriesID', 'SpatialResolution', 'TR', 'TE', 'FlipAngle', 'SliceThickness', 'SpacingBetweenSlices', 'BreastSide', 'QuadrantIQ', 'ClockStart', 'ClockEnd', 'LesionCenterPixelX', 'LesionCenterPixelY', 'LesionFindingType.Description', 'chosen_lesions_id', 'num_vox_sub1', 'volume_lesion_sub1', 'num_vox_sub2', 'volume_lesion_sub2', 'num_vox_sub3', 'volume_lesion_sub3','num_vo_sub4', 'volume_lesion_sub4']
...
try:
	for line in file_ids:
		...
		if(choseTodo == '1'):
			...
			irow_lesionSegment = create_SER(img_folder, LesionID, SeriesID, eID, StudyID, abspath_ExamID, row, dicomInfo, tablefile_name, tablewriter, irow_lesionSegment)
				
			# *****************************************************************************
			print "\n\nWriting output log to file"
