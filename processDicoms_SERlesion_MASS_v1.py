...
def create_SER(img_folder, LesionID, SeriesID, eID, StudyID, abspath_ExamID, row, dicomInfo, tablefile_name, tablewriter, irow_lesionSegment):
	...
	for i in range(1,5):
		...
		choseTodo = raw_input('Enter option to Update extent: \n\t1) 0 Ignore	\n\t1) Update \n\t input: ')
		if(choseTodo == '0'):
		...
		if(choseTodo == '1'):
			zMin_E = int(raw_input('Enter min int for updated Z extent: '))
			zMin_E = int(zMin_E) 
			zMax_E = int(raw_input('Enter max int for updated Z extent: '))
			zMax_E = int(zMax_E) 
			...
			thresh_sub.ReplaceInOff()
			thresh_sub.ReplaceOutOn()
			thresh_sub.ComputeInputUpdateExtent(xMin, xMax, yMin, yMax, zMin, zMax, xMin, xMax, yMin, yMax, zMin_E, zMax_E)
			thresh_sub.ThresholdBetween(0.35*uThre, uThre); 
			thresh_sub.SetInputConnection(sub_pre.GetOutputPort())
			thresh_sub.Update()
