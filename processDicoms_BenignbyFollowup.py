...
def get_slices_at_all_locs(img_folder, abspath_SeriesID, len_listSeries_files, StudyID, eID, SeriesID, choseSerie, listSeries_files, num_series, abspath_ExamID ):
	...
	for n in range(len_listSeries_files):
		...
		absp_fsID = 'Z:/Cristina/MassNonMass/BenignByFollowUp/'+img_folder+StudyID+'/'+eID+'/'+SeriesID[int(choseSerie)]+'/'+listSeries_files[n]
...
def get_locs_for_all_phases(img_folder, SeriesID, choseSerie, StudyID, eID, abspath_ExamID, len_listSeries_files, listSeries_files ):
	# enter folder of Series selection /examID/S_XXX 
	phases_series=[]
	chosen_phase = int(SeriesID[int(choseSerie)])
...
try:
	for line in file_ids:
		...
		SELECT     Patient.Patients.MRN, Patient.Patients.StudyNumber, Radiology.RadiologyReport.DicomExamNumber, Radiology.RadiologyReport.DateOfExam, 
                      Lesion.GeoDescription.Description AS Expr1, Lesion.FindingLocation.AdditionDescription, Lesion.GeoDescription.BreastSide, Lesion.FindingLocation.QuadrantID, 
                      ...
                      Lesion.FindingLocation.LesionCenterPixelY, Visit.ImagingVisitType.VisitTitle, Lesion.LesionFindingType.Description, Visit.ImagingVisits.VisitReasonTypeID, 
                      Radiology.RadiologyReport.ACRScoreID, Radiology.Lesions.HistopathTypeID, Pathology.PathoResultType.PathoResultTypeID, 
                      Pathology.PathoResultType.Description AS Expr2
                FROM         Radiology.RadiologyReport INNER JOIN
                      ...
                      Visit.VisitReasonType ON Visit.ImagingVisits.VisitReasonTypeID = Visit.VisitReasonType.VisitReasonTypeID CROSS JOIN
                      Pathology.PathoResultType
		...
		chdirname='Z:/Cristina/MassNonmass/BenignByFollowUp/'
		os.chdir(chdirname)  
		print os.getcwd()
		...
		while(choseSerie!='x'):
			...
			abspath_SeriesID = 'Z:/Cristina/MassNonMass/BenignByFollowUp/'+path_SeriesID
			...
			abspath_path_filenameID = 'Z:/Cristina/MassNonMass/BenignByFollowUp/'+path_filenameID
			...
			# *****************************************************************************
			print "Current folder time"
			chdirname='Z:/Cristina/MassNonmass/BenignByFollowUp/'+img_folder
			os.chdir(chdirname)  
			print os.getcwd()
			...
			if len(row) != 0:
				...
				file_summary_cases.write(str("ACRScore: ")+str( row[16])+'\n')
			...
			if (flag_dyn == 1):				
				os.chdir('DynPhases')
				abspath_SeriesID='Z:/Cristina/MassNonmass/BenignByFollowUp/'+img_folder+StudyID+'/'+eID+'/'+'DynPhases/'
