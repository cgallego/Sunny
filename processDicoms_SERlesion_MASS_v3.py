...
def display(image, image_pos_pat, image_ori_pat):
	global xMin, xMax, yMin, yMax, zMin, zMax, xSpacing, ySpacing, zSpacing, interactor, actions, reslice, interactorStyle, window
	...
	return origin, zImagePlaneWidget.GetSliceIndex()
		 
def display_pick(image, spatial_res, slice_thickn, image_pos_pat, image_ori_pat, dicomReader, zplane):
	global origExt, xMin, xMax, yMin, yMax, zMin, zMax, xSpacing, ySpacing, zSpacing, origin
...
def create_SER(selectMethod, numberS, img_folder, LesionID, SeriesID, eID, StudyID, abspath_ExamID, row, dicomInfo, tablefile_name, tablewriter, irow_lesionSegment):
	...
	if(selectMethod == '0'):
		...
		if(list_files[0]!='DIRCONTENTS.txt'):
		...
		#send to display
		[origin, zplane] = display(dicomReader.GetOutput(), image_pos_pat, image_ori_pat)
	...
	if(selectMethod == '1'):
		...
		# select z plane
		[origin, zplane] = display(im, image_pos_pat, image_ori_pat)
	...
	for i in range(1,numberS):
		...
		if(selectMethod == '0'):
			...
			sub_preMat.SetInput1(change_image.GetOutput())
		...
		if(i == 1 and selectMethod == '0'):
			sub_pre_early = vtk.vtkImageData()
			sub_pre_early = sub_preMat.GetOutput()
		...
		if(i == numberS-1 and selectMethod == '0'):
			sub_pre_late = vtk.vtkImageData()
			sub_pre_late = sub_preMat.GetOutput()
		...
		seeds = display_pick(sub_pre, spatial_res, slice_thickn, image_pos_pat, image_ori_pat, dicomReader, zplane)
		file_Seeds.close()
		...
		thresh_sub.ThresholdBetween(0.4*uThre, uThre); 
	...
	for line in file_ids:
		...
		while(choseTodo != 'x'):
			if(choseTodo == '1'):	
				...
				[origin, z_visualSerie] = display(dicomReader.GetOutput(), image_pos_pat_visual_SerieID, image_ori_pat_visual_SerieID)
